...
class RSSHandler:
    ...
    def execute(self, command: str) -> str:
        ...
        feed_entries_content_size = self.queue_proxy.token.get_content_count(
            feed_entries_content
        )

        # the `base_limit` represents a percentage of the context window size
        # the context window size is represented as the `upper_limit`
        if feed_entries_content_size > self.queue_proxy.token.base_limit:
            return f"The content is too large to display. It has been saved to a file: {cache_path}"

        return feed_entries_content
