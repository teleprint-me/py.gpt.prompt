...
# This script is specifically for downloading the base and chat models.
# There are 3 sets: The base model, the instruct model, and the python model
# scripts/llama_meta.sh
...
do
    ...
    echo "Preparing to download ${MODEL_PATH}"
    ...
    do
        if [ ! -f ${TARGET_FOLDER}"/${MODEL_PATH}/consolidated.${s}.pth" ]; then
            ...
            echo "${MODEL_PATH}/consolidated.${s}.pth already exists, skipping download."
        fi
    done
    ...
    wget ${PRESIGNED_URL/'*'/"tokenizer_checklist.chk"} -O ${TARGET_FOLDER}"/${MODEL_PATH}/tokenizer_checklist.chk"
